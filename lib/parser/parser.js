...
LokeParser.prototype["identifier"] = function $identifier() {
    var id;
    return this._rule("token", true, [ "Identifier" ], null, this["token"]) && (id = this._getIntermediate(), true) && this._exec(new Symbol(id));
};

LokeParser.prototype["seperator"] = function $seperator() {
    return this._rule("token", true, [ "Seperator" ], null, this["token"]);
};

LokeParser.prototype["newline"] = function $newline() {
    return this._rule("token", true, [ "Newline" ], null, this["token"]);
};
...
LokeParser.prototype["terminator"] = function $terminator() {
    return (this._atomic(function() {
        return this._rule("token", true, [ "Terminator" ], null, this["token"]);
    }) || this._atomic(function() {
        return this._rule("newline", false, [], null, this["newline"]);
...
LokeParser.prototype.token = function() {
    ...
    if (Array.isArray(current) && current.length == 2) return this._skip();
    return false;
};